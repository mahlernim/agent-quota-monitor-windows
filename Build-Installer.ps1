...
# The next installer deletes files listed here that it no longer ships.
$manifest = Join-Path $output 'installed-files.txt'
$files = @(Get-ChildItem -LiteralPath $bundle -Recurse -File | ForEach-Object { $_.FullName.Substring($bundle.Length + 1) }) + 'installed-files.txt'
$files | Sort-Object | Set-Content -LiteralPath $manifest -Encoding ascii
& $iscc "/DAppVersion=$Version" "/DBundleDir=$bundle" "/DOutputDir=$output" "/DManifestFile=$manifest" (Join-Path $PSScriptRoot 'packaging/windows-installer.iss')
