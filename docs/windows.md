...
Colors identify agents using blue-green shades. Yellow means quota remaining is below half the remaining portion of the time window. Red means it is below one quarter. Missing timing data never generates a projected warning. Percentages show at most one decimal.
