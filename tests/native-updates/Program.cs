...
await service.Check(); Assert(handler.Count == 1,"check interval");
now=now.AddHours(2).AddMinutes(59); await service.Check(); Assert(handler.Count == 1,"no automatic check before three hours");
now=now.AddMinutes(1); await service.Check(); Assert(handler.Count == 2 && UpdateService.AutomaticInterval == TimeSpan.FromHours(3),"automatic check after three hours");
...
await restart.Check(); Assert(restart.Available is null,"restart retains the check interval");
